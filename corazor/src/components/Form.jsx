...
const Form = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post('https://corazor-server-8da9.onrender.com/api/v1/auth/register', {name, email, message,phone,services});
  ...
  return (
    <section ref={ref} id="Contact Us" className={layout.section}>
      ...
      >
        ...
        <input className="input" type="text" required placeholder="Name" name="name" value={name} onChange={(e) => setName(e.target.value)} />
          <input className="input" type="email" required placeholder="Email" name="email" value={email} onChange={(e) => setEmail(e.target.value)} />
          <input className="input" type="number" required placeholder="Phone Number" name="Phone" value={phone} onChange={(e) => setPhone(e.target.value)} />
          ...
          <textarea className="textarea" rows={4} required placeholder="discribe your idea..." name="message" value={message} onChange={(e) => setMessage(e.target.value)} />
          {
            submitting==true?<button className="button">Submitiing...</button>:<button className="button"  onClick={(e) => handleSubmit(e)}>Submit</button>
          }
      {error && <span style={{color:"red"}}>Trouble sending email !!</span>}
      {success && <span style={{color:"#00FF00"}}>Your details was sent successfully.</span>}
        </form>
      </motion.div>
      <motion.div
        initial={{ y: +100 }}
